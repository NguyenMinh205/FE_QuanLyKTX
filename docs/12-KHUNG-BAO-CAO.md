...
| **1.2 Cơ sở lý thuyết** | `05` mục 1, 2 | **Viết mới** — lý thuyết nền về React, Node, REST, JWT, RBAC (tra cứu tài liệu chính thức, có trích dẫn) |
...
| **3.1 Kiến trúc** | `05` mục 1, 2, 3 | Mục 3.1.4 "Lựa chọn công nghệ và lý do": lấy lập luận từ `14` mục 10 |
| **3.2 Cơ sở dữ liệu** | `04` mục 1, 2, 3, 7 | Xuất ERD thành ảnh chất lượng cao |
| **3.3 API** | `06` mục 1, 15 + chọn lọc ví dụ | Không đưa toàn bộ 53 endpoint vào thân bài — để ở Phụ lục B |
| **3.4 Phân quyền & bảo mật** | `07` mục 1, 2, 3, 4, 5 | – |
| **3.5 Giao diện** | `08` mục 1, 2, 4, 6 | Thay wireframe ASCII bằng ảnh chụp màn hình thật |
| **4.1 Môi trường phát triển** | `05` mục 2 + `13` mục 3 | Ảnh chụp VS Code, cấu hình |
| **4.2 Cấu trúc mã nguồn** | `05` mục 3 | Ảnh chụp cây thư mục thật |
| **4.3 Cài đặt chức năng** | `08` mục 6 | **Viết mới** — mỗi chức năng: mô tả + ảnh chụp + đoạn code tiêu biểu |
| **4.4 Vấn đề kỹ thuật nổi bật** | `03` mục 3, `04` mục 7, `14` mục 4.6 và 4.10 | **Phần ăn điểm nhất** — xem mục 4 bên dưới |
...
| 3.1 | Kiến trúc tổng thể hệ thống | `05` mục 1 |
| 3.2 | Sơ đồ thực thể quan hệ (ERD) | `04` mục 1 |
| 3.3 | Mô hình phân quyền RBAC | `07` mục 1 |
| 3.4 | Luồng xác thực JWT | `07` mục 4 |
| 3.5 | Sitemap hệ thống | `08` mục 2 |
| 3.6 | Cấu trúc thư mục frontend | `05` mục 3.1 |
| 3.7 | Cấu trúc thư mục backend | `05` mục 3.2 |
...
2. **Phân tích các giải pháp:**
   ...
   - Khóa hàng bi quan (`SELECT ... FOR UPDATE`) + partial unique index → đúng, nhưng cần SQL thô, cần migration thủ công và **chỉ chạy trên PostgreSQL**.
...
4. **Kiểm chứng:** mô tả test case TC-72 (hai trình duyệt cùng nộp đơn) và kết quả.
...
[4]  PostgreSQL Global Development Group, "PostgreSQL 15 Documentation",
     https://www.postgresql.org/docs/15/, truy cập ngày 20/09/2026.
[5]  Prisma Data Inc., "Prisma ORM Documentation", https://www.prisma.io/docs,
...
2. Sinh viên đăng nhập → tra cứu giường trống → nộp đơn (1,5 phút)
...
|---------|---------------|
| "Vì sao chọn React và Node.js?" | Nêu lý do kỹ thuật (cùng ngôn ngữ JS cho cả FE/BE giảm chi phí chuyển đổi tư duy, hệ sinh thái lớn, phù hợp ứng dụng nhiều tương tác) — xem `05` mục 2. |
...
| "Dữ liệu sinh viên lấy từ đâu?" | Nhập thủ công hoặc import Excel; chưa tích hợp hệ thống đào tạo — đã nêu trong giả định (`01` mục 3.3). |
